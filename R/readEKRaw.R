...
#' @rdname readEKRaw
#'
readEKRaw <- function(f, t=1, endian="little", timeOffset=0, minTimeDiff=Inf, drop.out=FALSE, msg=TRUE, prenumt=10, na.rm=TRUE, cleartemp=FALSE){
	...
	readEKRaw_writeTemp <- function(data, tmpdir, f, pind, prenumt, rawtype){
		...
		if(length(channelLengths)){
			names(channelLengths) = paste("L", TSD::zeropad(seq_along(channelLengths),3),sep="")
			# Unlist the data and give names:
			data$pings = lapply(data$pings, unlist, use.names=FALSE)
			names(data$pings) = paste("V", TSD::zeropad(seq_along(data$pings),3),sep="")
